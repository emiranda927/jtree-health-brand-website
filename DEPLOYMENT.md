...
| Analytics | GA4 `G-8M90ZXZ1NW` via GTM `GTM-WGCMNLXH`, Consent Mode v2 (default: all denied) | ✅ wired + container published |
| Cookie banner | Custom `CookieConsent.astro`, deny-by-default, persists choice | ✅ verified |
...
7. Cookie banner: defaults denied; Accept → `analytics_storage: granted` + persisted ✅
...
(updated 2026-07-11: added the temp domain, removed the dead Flywheel entry —
**launch needs no CORS change**). Unset by design: `RITTEN_API_URL/KEY`, `TURNSTILE_SECRET`.
...
Do this after the designer's changes are merged and re-verified (§5):
...
5. 24–48 h later: GA4 → Admin → Events → mark `inquiry_submitted` as **Key Event**. Restore DNS TTLs.
6. Decommission: cancel the Squarespace **site** subscription (keep domain registration + Cloudflare DNS), cancel Flywheel Tiny if still active.
...
- Repo is under **git** (since 2026-07-11). Work on a branch; `npm run dev` to preview; `npm run build` must pass before deploy. Deploy: `npx vercel --prod --yes` (Eliseo's Vercel login) — or connect the repo to GitHub for auto-deploys (not set up yet).
