...
- **Watchdog**: `/api/watchdog` cron (Mon–Fri 6 PM ET) alerts `OWNER_ALERT_EMAIL` if no leads in 24 h — expect alerts to be meaningful only after launch.
  **2026-07-18 finding**: the watchdog sent nothing during the 07-12→07-18 form outage because a hand-touched row in the Leads sheet read back as a formatted date (`7/15/2026`-style), which the old string compare counted as a fresh lead forever (live probe: `{"checked":true,"count":1}` while the form was provably broken). Fixed in code (`Date.parse` + fail-loud alerts). Owner still must: (1) open the Leads sheet and fix/delete any row whose `submitted_at` isn't an ISO `2026-…T…Z` string — format column B as *Plain text*; (2) re-enter `OWNER_ALERT_EMAIL` in Vercel → `jtree-health` → Env Vars (it's a "sensitive" var, unreadable since 2026-05-05, so its value can't be audited — only overwritten); (3) after the next 22:00 UTC weekday run, confirm the run in Vercel → Cron Jobs and the send in Resend → Logs. Cron itself verified enabled via API (since 2026-05-05).
